...
from gui.plots import PlotContainer
from funcs import *
...
class MainApplication():
    ...
    def __init__(self,ConfigFile):
        ...
        # Create and empty GUI containing only the console, so it can show any error which might occur while loading the instruments
        self.CreateGUI(empty=1) 
        self.Initiate(ConfigFile)

    def Initiate(self,ConfigFile):
        ...
        print("Loading all instruments... ", end='')
        flag = self.LoadInstruments()
        if flag == 0: #if any error occurred while loading the instruments, flag is set to 0
            return
        print("All instruments loaded correctly.")
        ...
        print("Checking the user-specified post-processed data... ", end='')
        flag = self.LoadAndCheckPostProcessedData()
        if flag == 0: #if any error occurred while checking PostProcessedData, flag is set to 0
            return
        if flag == 2:
            print("No post-processed data was specified by the user in this .json file")
        if flag == 1:
            print("All user-specified post-processed data are valid.")

        if flag==1: #if the user specified any post-processed data, we add their names to the self.NameData list
            for Data in self.PostProcessedData:
                self.NameData.append(Data[0])
    ...
    def LoadInstruments(self):
        ...
        for (deviceID,device) in enumerate(self.deviceNames):
            if not(device in dirs):
                print("The device name '" + device + "' does not correspond to a valid subfolder inside the 'instruments' folder. Fix the problem in the .json file and restart the application.", file=sys.stderr)
                flag = 0
                break
            try:
                ...
                flag = 0
                traceback.print_exc()
                print("Error when trying to load the module for the device '" + device + "'. Check that the corresponding folder (inside the 'instruments' folder) contains a valid 'interface.py' file with an interface class. Fix the problem and restart the application.", file=sys.stderr)
                return flag
            ...
            for data in self.deviceDataToAcquire[deviceID]:
                if not(data in DeviceClass.interface.Data.keys()):
                    print("The data '"+data+"' is not a valid data name for the device '"+device+"'. According to the 'interface.py' file of this device,the valid data names are:", file=sys.stderr)
                    for key, val in DeviceClass.interface.Data.items():
                        print("-'{}'".format(key), file=sys.stderr)
                    print("Fix the problem in the .json file and restart the application.", file=sys.stderr)
                    flag = 0
        return flag

    def LoadAndCheckPostProcessedData(self):
        flag = 1
        ListNames = [] #This is a temporary list. We add all names of postprocessed data here, to check that no name is repeated
        if hasattr(self,'PostProcessedData'):
            for index,Data in enumerate(self.PostProcessedData):

                if (type(Data[0]) == str):
                    if Data[0] in ListNames:
                        msg = "\nThe name '" + Data[0] + "' is being used more than once as a name for postprocessed data. Names must be unique. Fix the problem in the .json file and restart the application."
                        flag = 0
                    else:
                        ListNames.append(Data[0])
                else:
                    flag =0
                    msg = "\nThe first element of post-processed data #" + str(index+1) + " is not a valid string. Fix the problem in the .json file and restart the application."
                
                if flag==0:
                    print(msg, file=sys.stderr)
                    break
                else:
                    msg,flag =  CheckListIsValidFormula(Data[1],self.NameData)
                    if flag==0:
                        print("\nA problem has occured with the post-processed data named '" + Data[0] + "':", file=sys.stderr)
                        print(msg, file=sys.stderr)
                        print("Fix the problem in the .json file and restart the application.", file=sys.stderr)
                        break
        else:
            flag = 2 # when flag = 2, there are no post-processed data
        return flag
    ...
    def LoadConfig(self,ConfigFile):
        ...
        with open(ConfigFile) as jsonfile:
            ...
            if 'PostProcessedData' in config.keys():
                self.PostProcessedData = config['PostProcessedData']
    ...
    def CreateGUI(self,empty=0):
        ...
        When empty = 1, we create an empty gui with the console only, in this way we can print any error that might occur when loading the instrument drivers
        '''
        ...
        if not(hasattr(self, 'ContainerForInstrumentFrames')):
        ...
        if empty==0:
            if not(hasattr(self, 'ContainerForSettingFrames')):
        ...
        if not(hasattr(self, 'BottomFrame')):
        ...
        if not(hasattr(self, 'OutputTextBox')):
    ...
    def DeleteStoredData(self):
        ...
        '''
        MsgBox = tk.messagebox.askquestion ('Delete all stored data','Are you sure you want to delete all data currently stored in the software?',icon = 'warning')
        if MsgBox == 'yes':
            self.InitializeStorageVariables()
            self.InitializeTemporaryVariables()
            self.UpdateAllPlots()
    ...
    def SaveStoredData(self):
        ...
        for h in self.NameData:
            header = header + h + d 
        for h in self.NameData[1:]: #We skip the first element, it corresponds to error on the acqsuisition number which is not meaninful
        ...
        A = np.concatenate((self.DataAcquired, self.DataAcquired_StdDev[:,1:]),axis=1)
    ...
    def ReadCurrentData(self):
        ...
        if hasattr(self,'PostProcessedData'): #If the user has specified post-processed data, we calculate it here and add it
            LenData = len(CurrentData)
            variables = {self.NameData[i+1]: CurrentData[i] for i in range(LenData)}  #Create a dictionary with the values of the data that we just acquired
            for Data in self.PostProcessedData:
                PostProcessedData = EvaluateFormula(Data[1],variables)
                CurrentData.append(PostProcessedData)  
    ...
    def StoreCurrentData(self):
        ...
        CurrentData = self.ReadCurrentData() #Read the current data from all instruments, and any user-defined postprocessed data
