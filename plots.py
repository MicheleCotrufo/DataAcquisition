...
class PlotObject(tk.Frame):
    def __init__(self,  master,  MainWindow, GetData, GetNameData, GetPlotConfig, SetPlotConfig, GetPlottingStyle, PlotSize, **kwargs):
        #GetData, GetNameData, GetPlotsConfig, SetPlotsConfig are functions, which normally are defines methods of the MainWindow object, and they let this PlotObject read the data and read/set the PlotsConfig
        ...
        #Create the figure
        self.fig = Figure(figsize=PlotSize, dpi=100)
...
class PlotContainer(tk.Frame): #Container for Plots, which is also a TK frame
    ...
    def __init__(self, master, MainWindow, PlotsSizes, NCols,**kwargs):
        super().__init__(master,**kwargs)
        self.master = master #= Containing Frame
        self.MainWindow = MainWindow #= MainWindow
        NumberPlots = len(PlotsSizes)
